...
gcloud run deploy "$SERVICE" \
  ...
  --set-env-vars="FIRESTORE_PROJECT_ID=${PROJECT},GOOGLE_CLOUD_PROJECT=${PROJECT},FIREBASE_PROJECT=${PROJECT},GCS_STAGING_BUCKET=${STAGING_BUCKET},PUBLISH_TRIGGER_ID=${TRIGGER_ID},PUBLISH_BRANCH=${PUBLISH_BRANCH},N_PER_RUN=${N_PER_RUN},SCAN_LIMIT=${SCAN_LIMIT},REANALYZE_ENABLED=${REANALYZE_ENABLED},ANTHROPIC_AGENT_ID=${ANTHROPIC_AGENT_ID},ANTHROPIC_ENV_ID=${ANTHROPIC_ENV_ID}" \
